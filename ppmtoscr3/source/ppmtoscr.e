...
DEF fname2:ARRAY OF CHAR,fname[50]:STRING
DEF verstring[30]:STRING,saveformat=0,checksum,ppmmem:ARRAY OF CHAR
...
fname2:=FilePart(scrfile)
dot:=InStr(fname2,'.')
IF dot<>-1 THEN StrCopy(fname,fname2,dot) ELSE StrCopy(fname,fname2)
StrAdd(fname,'          ')
...
IF saveformat=4
  IF quiet=0 THEN WriteF('Save Format: TZX\n')
ENDIF
...
IF saveformat=4
  IF quiet=0 THEN WriteF('Writing TZX header...\n')
	...
	FputC(fhand,$32)
	FputC(fhand,28) /* length 2 bytes */
	FputC(fhand,00) /* MSB */
	FputC(fhand,01)
	FputC(fhand,$FF)
	FputC(fhand,25) /* length of text below */
	Fputs(fhand,'Created with ppmtoscr 3.5')
...
col:=FindColor(cm,red,grn,blu,-1) -> 15
