...
void* NewSemaphore()  {
	...
	IExec->InitSemaphore(& ((semaphore*) sem2)->ss );
	return sem2 ;
} 
...
void SemLock(void* sem2)  {
	IExec->ObtainSemaphore(& ((semaphore*) sem2)->ss );
	return ;
}

void SemUnlock(void* sem2)  {
	IExec->ReleaseSemaphore(& ((semaphore*) sem2)->ss );
	return ;
}

BOOLEAN SemTryLock(void* sem2)  {
	BOOLEAN success; 
	success = - ((long) IExec->AttemptSemaphore(& ((semaphore*) sem2)->ss )!= (long) 0);
	return success ;
} 
...
struct Library* OpenLibrary_exec2(char* libName, ULONG version)  {
	...
	if( (ret = IExec->OpenLibrary(libName ,version ))) {
		...
		if( (void*) node == NULL ) { Throw(QuadChara(0,'B','U','G'), (char*) "target/PE/exec; OpenLibrary(); InitLibrary() has not been called for supplied library");}
		
		//handle OS4 library interface
		node->count = node->count + (long) 1;
		if( (void*) node->ifaceAddress[0] == NULL) {
			...
			Throw(QuadChara(0,'B','U','G'), (char*) "target/PE/exec; OpenLibrary(); SUPER call unexpectedly returned a different library compared to a previous call");
		}
	}
	return ret ;
} 
...
void CloseLibrary_exec2(struct Library* library)  {
	...
	//handle OS4 library interface
	if( (void*) node == NULL) {
		//Throw("BUG", 'target/PE/exec; CloseLibrary(); InitLibrary() has not been called for supplied library')
	} else {
		node->count = node->count - (long) 1;
...
void new_asl()  {
	...
	asl_semaphore_name = (char*) "asl.library";
	return ;
}
...
void new_asl2() {
	InitLibrary((char*) "asl.library", (struct Interface **) &IAsl);
	return ;
}
...
try {
	...
	wptr2 = IIntuition->OpenWindowTagList((struct NewWindow*) NULL ,temp_ARRAY_OF_tagitem  );
	/*unused*/ sflags = 0;
	stdrast = wptr2->RPort;
} catch(...) {}
...
try {
	...
	sptr = IIntuition->OpenScreenTagList((struct NewScreen*) NULL ,temp_ARRAY_OF_tagitem  );
	/*unused*/ sflags = 0;
...
void new_boopsi() {
	UtilityBase = OpenLibrary_exec2((char*) "utility.library", (ULONG) 39);
	if( (void*) UtilityBase==NULL ) { CleanUp(RETURN_ERROR);}
	return ;
}

void end_boopsi() {
	CloseLibrary_exec2(UtilityBase);
	return ;
}
...
void new_datatypes() {
	InitLibrary((char*) "datatypes.library", (struct Interface **) &IDataTypes);
	return ;
}
...
/*** Files:/Projects/PDHFIC/dt2scr2_asl_aros_3.3 ***/
...
void new_dt2scr2_asl_aros_3_3() ;
void end_dt2scr2_asl_aros_3_3() ;
...
try {
	...
	new_wb();
	new_wb2();
	new_dt2scr2_asl_aros_3_3();

Rnd(-1);

StrCopy(verstring,(char*) "$VER: PDHFIC 3.3 (11.10.2009)");
...
// WriteF('\s\n',rargs)
    ...
    if( (long) (DataTypesBase=OpenLibrary_exec2((char*) "datatypes.library", (ULONG) 39))==(long) 0 ) { Raise(ERR_LIB);}

    if( (long) (rexxsysbase=OpenLibrary_exec2((char*) "rexxsyslib.library", (ULONG) 37))==(long) 0 ) { Raise(ERR_LIB);}
...
ppmtoscr();
    ...
    if( (long) (AslBase=OpenLibrary_exec2((char*) "asl.library",(ULONG) 37))==(long) 0 ) { Raise(ERR_ASL);}
    if( (void*) (IconBase=OpenLibrary_exec2((char*) "icon.library",(ULONG) 33))==NULL ) { Raise(ERR_ICON);}
    if( (void*) (GadToolsBase=OpenLibrary_exec2((char*) "gadtools.library",(ULONG) 37))==NULL ) { Raise(ERR_GTLS);}
    if( (void*) (WorkbenchBase=OpenLibrary_exec2((char*) "workbench.library",(ULONG) 37))==NULL ) { Raise(ERR_WBLB);}
...
// Let's get the blocks in the right order, eh chaps?
    ...
    IDOS->FPuts((BPTR) fhand ,"Converted with PDHFIC v3.3" );
...
if( tzxarray[1]) {
    ...
    if( EstrLen(objanno)!=(long) NULL) {
        ...
        IDOS->FPuts((BPTR) fhand ,"Screen created with PDHFIC 3.3" );
    }
...
if( gifsave) {
    ...
    StrAdd(tmpstr,(char*) "\"");
    (BOOL)(0!=IDOS->Execute(tmpstr ,(BPTR) (long) NULL ,(BPTR) (long) NULL ));
    (BOOL)(0!=IDOS->DeleteFile(scrfile ));
    StrCopy(scrfile,gifname);
}
...
ppm=FALSE;
        // Check for PPM
            if( (okay=(BOOL)(0!=IDOS->Examine((BPTR) lock ,& finfo )))) {
...
if( clipboard==0) {
    ...
    if( (o=(long) IDataTypes->NewDTObjectA(filename ,(struct TagItem*) tempGlobal_ILIST3  ))==0) {
/*
...
*/                      
    ...
    if( (o=(long) IDataTypes->NewDTObjectA((char*) clipunit ,(struct TagItem*) tempGlobal_ILIST4  ))==0) {
...
*/
    ...
    if( scaleornot) {
		...
		IIntuition->IDoMethodA((Object*) o , (Msg)(long) tempGlobal_ARRAY_OF_pdtscale  );
...
//    rgbs:=ppmmem
	...
	IIntuition->IDoMethodA((Object*) o , (Msg)(long) tempGlobal_ARRAY_OF_pdtblitpixelarray  );
...
if( fromcli) {
    if( (long) (IconBase=OpenLibrary_exec2((char*) "icon.library",(ULONG) 36))==(long) 0 ) { Raise(ERR_ICNN);}
...
if( fromcli==0 ) { IDOS->CurrentDir((BPTR) olddir );}
	return ;
...
// gadg 1
  ...
  gads[GADG_LOADSTR]=(long) IGadTools->CreateGadgetA((ULONG) TEXT_KIND ,(struct Gadget*) gad ,(struct NewGadget*) tempGlobal_ARRAY_OF_newgadget  ,tempGlobal_ARRAY_OF_tagitem  );
...
tempGlobal_ARRAY_OF_tagitem2 [6].ti_Data = (ULONG) (pixsep*(long) 6+(long) ((long) mysc->Font->ta_YSize*5)+tbar+(long) 10);

tempGlobal_ARRAY_OF_tagitem2 [14].ti_Data = (ULONG) (long) zoompos;

wptr=IIntuition->OpenWindowTagList((struct NewWindow*) NULL ,tempGlobal_ARRAY_OF_tagitem2  );
} else {
tempGlobal_ARRAY_OF_tagitem3 [1].ti_Data = (ULONG) (long) glist[0];
tempGlobal_ARRAY_OF_tagitem3 [4].ti_Data = (ULONG) (pixsep*(long) 6+(long) ((long) mysc->Font->ta_YSize*5)+tbar+(long) 10);
tempGlobal_ARRAY_OF_tagitem3 [12].ti_Data = (ULONG) (long) zoompos;
wptr=IIntuition->OpenWindowTagList((struct NewWindow*) NULL ,tempGlobal_ARRAY_OF_tagitem3  );
}
...
(BOOL)(0!=IDOS->AddPart(filename ,filereq->fr_File ,(ULONG) (long) 1024 ));
...
StrCopy(scrfile,drw);
(BOOL)(0!=IDOS->AddPart(scrfile ,fname ,(ULONG) (long) 1024 ));
...
if( strlen(savereq->fr_File )==0) {
    ...
    (BOOL)(0!=IDOS->AddPart(scrfile ,savereq->fr_File ,(ULONG) (long) 1024 ));
}
...
//        WriteF('\s\n',argptr.name)
        ...
        (BOOL)(0!=IDOS->NameFromLock(argptr->wa_Lock ,(char*) filebuf ,1024 ));
        StrCopy(filename,(char*) filebuf);
        StrCopy(loaddrw,filename);
          (BOOL)(0!=IDOS->AddPart(filename ,argptr->wa_Name ,(ULONG) (long) 1024 ));
...
/*    SizeWindow(wptr,0,-((pixsep*8)+(mysc.font.ysize*6))) *//* 240,0 */
    IIntuition->ChangeWindowBox(wptr ,wptr->LeftEdge ,wptr->TopEdge ,270 ,(pixsep*(long) 6+(long) ((long) mysc->Font->ta_YSize*5)+tbar+(long) 10) );
...
(BOOL)(0!=IDOS->AddPart(name ,finfo->fib_FileName ,(ULONG) (long) 1024 ));
...
pwptr=IIntuition->OpenWindowTagList((struct NewWindow*) NULL ,tempGlobal_ARRAY_OF_tagitem4  );
...
(BOOL)(0!=IDOS->AddPart(tmpstr2 ,arexxreq->fr_File ,(ULONG) (long) 1024 ));
...
void new_dt2scr2_asl_aros_3_3()  {
	...
	tempGlobal_ARRAY_OF_easystruct [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct2 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct2 [0].es_TextFormat = (char*) "PDHFIC wants to write a\ntemporary file, but it\nalready exists!";
	tempGlobal_ARRAY_OF_easystruct2 [0].es_GadgetFormat = (char*) "Overwrite|Cancel";
	...
	tempGlobal_ARRAY_OF_easystruct3 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct3 [0].es_TextFormat = (char*) "Warning!\n\nA .header for this\nfile already exists!";
	tempGlobal_ARRAY_OF_easystruct3 [0].es_GadgetFormat = (char*) "Overwrite|Cancel";
	...
	tempGlobal_ARRAY_OF_easystruct4 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct4 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct5 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct5 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct6 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct6 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct7 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct7 [0].es_TextFormat = (char*) "Clipboard data not ILBM";
	tempGlobal_ARRAY_OF_easystruct7 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct8 [0].es_Title = (char*) "Welcome...";
	tempGlobal_ARRAY_OF_easystruct8 [0].es_TextFormat = (char*) "Hello.  I\'m Nigel Blenkinsopp,\nthe long lost explorer of the\nfar east.";
	tempGlobal_ARRAY_OF_easystruct8 [0].es_GadgetFormat = (char*) "Pleased to meet you";
	...
	tempGlobal_ARRAY_OF_newgadget [0].ng_GadgetText = (char*) "Input:";
	...
	tempGlobal_ARRAY_OF_newgadget2 [0].ng_GadgetText = (char*) "Select";
	...
	tempGlobal_ARRAY_OF_newgadget3 [0].ng_GadgetText = (char*) "Output:";
	...
	tempGlobal_ARRAY_OF_newgadget4 [0].ng_GadgetText = (char*) "Select";
	...
	tempGlobal_ARRAY_OF_newgadget5 [0].ng_GadgetText = (char*) "Convert!";
	...
	tempGlobal_ARRAY_OF_newgadget6 [0].ng_GadgetText = (char*) "Save as";
	...
	tempGlobal_ARRAY_OF_newgadget7 [0].ng_GadgetText = (char*) "ROM Remap";
	...
	tempGlobal_ARRAY_OF_newgadget8 [0].ng_GadgetText = (char*) "Scale Pic";
	...
	tempGlobal_ARRAY_OF_newgadget9 [0].ng_GadgetText = (char*) "Smooth";
	...
	tempGlobal_ARRAY_OF_newgadget10 [0].ng_GadgetText = (char*) "Dither";
	...
	tempGlobal_ARRAY_OF_newmenu [0].nm_Label = (char*) "Project";
	...
	tempGlobal_ARRAY_OF_newmenu [1].nm_Label = (char*) "About...";
	tempGlobal_ARRAY_OF_newmenu [1].nm_CommKey = (char*) "A";
	...
	tempGlobal_ARRAY_OF_newmenu [3].nm_Label = (char*) "Quit...";
	tempGlobal_ARRAY_OF_newmenu [3].nm_CommKey = (char*) "Q";
	...
	tempGlobal_ARRAY_OF_newmenu [4].nm_Label = (char*) "Picture";
	...
	tempGlobal_ARRAY_OF_newmenu [5].nm_Label = (char*) "Select Source...";
	tempGlobal_ARRAY_OF_newmenu [5].nm_CommKey = (char*) "S";
	...
	tempGlobal_ARRAY_OF_newmenu [6].nm_Label = (char*) "From Clipboard";
	tempGlobal_ARRAY_OF_newmenu [6].nm_CommKey = (char*) "V";
	...
	tempGlobal_ARRAY_OF_newmenu [7].nm_Label = (char*) "Display...";
	...
	tempGlobal_ARRAY_OF_newmenu [9].nm_Label = (char*) "Select Destination...";
	tempGlobal_ARRAY_OF_newmenu [9].nm_CommKey = (char*) "D";
	...
	tempGlobal_ARRAY_OF_newmenu [10].nm_Label = (char*) "Display...";
	...
	tempGlobal_ARRAY_OF_newmenu [12].nm_Label = (char*) "Information...";
	tempGlobal_ARRAY_OF_newmenu [12].nm_CommKey = (char*) "I";
	...
	tempGlobal_ARRAY_OF_newmenu [13].nm_Label = (char*) "Settings";
	...
	tempGlobal_ARRAY_OF_newmenu [14].nm_Label = (char*) "Advanced Options";
	tempGlobal_ARRAY_OF_newmenu [14].nm_CommKey = (char*) "O";
	...
	tempGlobal_ARRAY_OF_newmenu [15].nm_Label = (char*) "Use Screen";
	...
	tempGlobal_ARRAY_OF_newmenu [16].nm_Label = (char*) "Warn Overwrite";
	...
	tempGlobal_ARRAY_OF_newmenu [17].nm_Label = (char*) "Intelligent Filtering";
	...
	tempGlobal_ARRAY_OF_newmenu [18].nm_Label = (char*) "Automatic Naming";
	...
	tempGlobal_ARRAY_OF_newmenu [19].nm_Label = (char*) "Progress Window";
	...
	tempGlobal_ARRAY_OF_newmenu [21].nm_Label = (char*) "Map Extra Colours To";
	...
	tempGlobal_ARRAY_OF_newmenu [22].nm_Label = (char*) "Auto";
	...
	tempGlobal_ARRAY_OF_newmenu [23].nm_Label = (char*) "Ink";
	...
	tempGlobal_ARRAY_OF_newmenu [24].nm_Label = (char*) "Paper";
	...
	tempGlobal_ARRAY_OF_newmenu [25].nm_Label = (char*) "Attributes";
	...
	tempGlobal_ARRAY_OF_newmenu [26].nm_Label = (char*) "ZXDT Palette";
	...
	tempGlobal_ARRAY_OF_newmenu [27].nm_Label = (char*) "Dithering Method";
	...
	tempGlobal_ARRAY_OF_newmenu [28].nm_Label = (char*) "Ordered";
	tempGlobal_ARRAY_OF_newmenu [28].nm_CommKey = (char*) "C";
	...
	tempGlobal_ARRAY_OF_newmenu [29].nm_Label = (char*) "Random";
	tempGlobal_ARRAY_OF_newmenu [29].nm_CommKey = (char*) "R";
	...
	tempGlobal_ARRAY_OF_newmenu [30].nm_Label = (char*) "Dither SimpleMap";
	...
	tempGlobal_ARRAY_OF_newmenu [31].nm_Label = (char*) "Remap After Dither";
	...
	tempGlobal_ARRAY_OF_newmenu [33].nm_Label = (char*) "TZX Blocks";
	...
	tempGlobal_ARRAY_OF_newmenu [34].nm_Label = (char*) "Archive Info";
	...
	tempGlobal_ARRAY_OF_newmenu [36].nm_Label = (char*) "Standard";
	...
	tempGlobal_ARRAY_OF_newmenu [37].nm_Label = (char*) "Custom";
	...
	tempGlobal_ARRAY_OF_newmenu [38].nm_Label = (char*) "TZX Custom Block Border";
	...
	tempGlobal_ARRAY_OF_newmenu [39].nm_Label = (char*) "Black";
	tempGlobal_ARRAY_OF_newmenu [39].nm_CommKey = (char*) "0";
	...
	tempGlobal_ARRAY_OF_newmenu [40].nm_Label = (char*) "Blue";
	tempGlobal_ARRAY_OF_newmenu [40].nm_CommKey = (char*) "1";
	...
	tempGlobal_ARRAY_OF_newmenu [41].nm_Label = (char*) "Red";
	tempGlobal_ARRAY_OF_newmenu [41].nm_CommKey = (char*) "2";
	...
	tempGlobal_ARRAY_OF_newmenu [42].nm_Label = (char*) "Cyan";
	tempGlobal_ARRAY_OF_newmenu [42].nm_CommKey = (char*) "3";
	...
	tempGlobal_ARRAY_OF_newmenu [43].nm_Label = (char*) "Green";
	tempGlobal_ARRAY_OF_newmenu [43].nm_CommKey = (char*) "4";
	...
	tempGlobal_ARRAY_OF_newmenu [44].nm_Label = (char*) "Magenta";
	tempGlobal_ARRAY_OF_newmenu [44].nm_CommKey = (char*) "5";
	...
	tempGlobal_ARRAY_OF_newmenu [45].nm_Label = (char*) "Yellow";
	tempGlobal_ARRAY_OF_newmenu [45].nm_CommKey = (char*) "6";
	...
	tempGlobal_ARRAY_OF_newmenu [46].nm_Label = (char*) "White";
	tempGlobal_ARRAY_OF_newmenu [46].nm_CommKey = (char*) "7";
	...
	tempGlobal_ARRAY_OF_newmenu [47].nm_Label = (char*) "Write .header";
	...
	tempGlobal_ARRAY_OF_newmenu [49].nm_Label = (char*) "Snapshot Window";
	...
	tempGlobal_ARRAY_OF_newmenu [50].nm_Label = (char*) "Save Configuration";
	...
	tempGlobal_ARRAY_OF_newmenu [51].nm_Label = (char*) "Help";
	...
	tempGlobal_ARRAY_OF_newmenu [52].nm_Label = (char*) "Selected Output Format...";
	...
	tempGlobal_ARRAY_OF_newmenu [53].nm_Label = (char*) "Relieving Boredom...";
	tempGlobal_ARRAY_OF_newmenu [53].nm_CommKey = (char*) "B";
	...
	tempGlobal_ARRAY_OF_newmenu [54].nm_Label = (char*) "ARexx";
	...
	tempGlobal_ARRAY_OF_newmenu [55].nm_Label = (char*) "Execute Script...";
	tempGlobal_ARRAY_OF_newmenu [55].nm_CommKey = (char*) "E";
	...
	tempGlobal_ARRAY_OF_tagitem2 [12].ti_Data = (ULONG) (long) "PDHFIC 3.3  \251 1998-2001,2008-2009 Unsatisfactory Software";
	...
	tempGlobal_ARRAY_OF_tagitem3 [10].ti_Data = (ULONG) (long) "PDHFIC 3.3  \251 1998-2001,2008-2009 Unsatisfactory Software";
	...
	tempGlobal_ARRAY_OF_easystruct9 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct9 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct10 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct10 [0].es_TextFormat = (char*) "ARexx port could\nnot be allocated";
	tempGlobal_ARRAY_OF_easystruct10 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct11 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct11 [0].es_TextFormat = (char*) "Warning!\n\nThat file already exists,\nand will be overwritten!";
	tempGlobal_ARRAY_OF_easystruct11 [0].es_GadgetFormat = (char*) "Overwrite|Cancel";
	...
	tempGlobal_ARRAY_OF_easystruct12 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct12 [0].es_TextFormat = (char*) "GIF\n\nThis saver is a bit special: it\nuses SCR2GIF to output a Spectrum\ntype image, but suitable for\nweb pages.\n\nSize: <6912\n\nUsed by: WWW and a decreasing\namount of other programs.";
	tempGlobal_ARRAY_OF_easystruct12 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct13 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct13 [0].es_TextFormat = (char*) "SCR\n\nThis is simply the raw data taken\nfrom the Spectrum\'s memory at\naddress 16384.\n\nSize: 6912\n\nUsed by: ZX Datatype, ZXAM";
	tempGlobal_ARRAY_OF_easystruct13 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct14 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct14 [0].es_TextFormat = (char*) "ZX82\n\nThis format is really a disk-based\nformat, as it does not contain\nan original Spectrum tape header.\n\nSize: 6924\n\nUsed by: Speculator, xfs, ZXMIT\n\nMore information available from\nSpeculator documentation";
	tempGlobal_ARRAY_OF_easystruct14 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct15 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct15 [0].es_TextFormat = (char*) "BYTES (.header/.bytes)\n\nThese are nothing other than\nindividual blocks of data as a\nSpectrum would save to tape.\n\nSize: 19 (.header), 6914 (.bytes)\n\nUsed by: Peter McGavin\'s Spectrum, ZXAM";
	tempGlobal_ARRAY_OF_easystruct15 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct16 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct16 [0].es_TextFormat = (char*) "TAP (Z80 Tape Image)\n\nTAPs are tape files which can store\nseveral data blocks in one file.\n\nSize: 6937\n\nUsed by: ZXAM, ASp";
	tempGlobal_ARRAY_OF_easystruct16 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct17 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct17 [0].es_TextFormat = (char*) "+3DOS\n\nThis is a +3DOS file, intended for\nuse on +3 disks\n\nSize: 7040\n\nUsed by: +3";
	tempGlobal_ARRAY_OF_easystruct17 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct18 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct18 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_easystruct19 [0].es_Title = (char*) "PDHFIC";
	tempGlobal_ARRAY_OF_easystruct19 [0].es_TextFormat = (char*) "Hello!";
	tempGlobal_ARRAY_OF_easystruct19 [0].es_GadgetFormat = (char*) "OK";
	...
	tempGlobal_ARRAY_OF_newgadget11 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget12 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget13 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget14 [0].ng_GadgetText = (char*) "Bright:";
	...
	tempGlobal_ARRAY_OF_newgadget15 [0].ng_GadgetText = (char*) "Colour:";
	...
	tempGlobal_ARRAY_OF_newgadget16 [0].ng_GadgetText = (char*) "White:";
	...
	tempGlobal_ARRAY_OF_newgadget17 [0].ng_GadgetText = (char*) "Red/Yel:";
	...
	tempGlobal_ARRAY_OF_newgadget18 [0].ng_GadgetText = (char*) "Blu/Cyn:";
	...
	tempGlobal_ARRAY_OF_newgadget19 [0].ng_GadgetText = (char*) "Grn/Mag:";
	...
	tempGlobal_ARRAY_OF_newgadget20 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget21 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget22 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget23 [0].ng_GadgetText = (char*) "Flash: Red";
	...
	tempGlobal_ARRAY_OF_newgadget24 [0].ng_GadgetText = (char*) "Green";
	...
	tempGlobal_ARRAY_OF_newgadget25 [0].ng_GadgetText = (char*) "Blue";
	...
	tempGlobal_ARRAY_OF_newgadget26 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget27 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_newgadget28 [0].ng_GadgetText = (char*) "";
	...
	tempGlobal_ARRAY_OF_tagitem4 [11].ti_Data = (ULONG) (long) "PDHFIC 3.3  \251 1998-2001,2008-2009 Unsatisfactory Software";
