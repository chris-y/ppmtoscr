...
StrCopy(verstring,'$VER: PDHFIC 3.3 (11.10.2009)')
...
PROC ppmtoscr()
DEF o=50,p=0,tzxstrs,tzxlens,specname2[256]:STRING,f
...
/* was StrAdd(specname,'          ')
This (and the stuff above) is broken - the name on the Spectrum
side should be name without ext and the rest of the characters
blanked out.
In 3.3, it isn't.  3.2 works (on OS4) but breaks on AROS,
therefore 3.3 not released for OS4 until this is fixed.
*/
...
-> Let's get the blocks in the right order, eh chaps?
    ...
    Fputs(fhand,'Converted with PDHFIC v3.3')
    FputC(fhand,13)
...
ENDIF
    ...
    ELSE
        ...
        Fputs(fhand,'Screen created with PDHFIC 3.3')
    ENDIF
...
*/
    ...
    IF scaleornot
		...
		doMethodA(o,[PDTM_SCALE,256,192,0]:pdtscale)
...
->    rgbs:=ppmmem
	...
	doMethodA(o,[PDTM_READPIXELARRAY,ppmmem,PBPAFMT_RGB,256*3,0,0,256,192]:pdtblitpixelarray)
...
IF forcenofilter THEN nofilter:=1
...
IF fromcli=0 THEN CurrentDir(olddir) -> check this is ok!
...
IF forcenofilter THEN filtermen:=(CHECKIT OR MENUTOGGLE OR NM_ITEMDISABLED)
...
wptr:=OpenWindowTagList(NIL,[WA_TITLE,'PDHFIC',
                           ...
                           WA_WIDTH,     270,   
                           WA_HEIGHT,((pixsep*6)+(mysc.font.ysize*5)+tbar+10),
                           WA_DRAGBAR,   TRUE,  WA_DEPTHGADGET,   TRUE,
                           WA_ACTIVATE,  TRUE,  WA_CLOSEGADGET,   TRUE,
                           WA_NEWLOOKMENUS, TRUE, ->WA_AUTOADJUST,TRUE,
                           WA_SCREENTITLE,'PDHFIC 3.3  © 1998-2001,2008-2009 Unsatisfactory Software',
        ...
        REPEAT
            ...
            IF arexxmsg<>NIL
                ...
                IF StrCmp(rexxcmd,'HELLO')
                    ...
                    StrCopy(result,'© 1998-2001, 2008-2009 Unsatisfactory Software')
                ENDIF
...
/*    SizeWindow(wptr,0,-((pixsep*8)+(mysc.font.ysize*6))) *//* 240,0 */
    ChangeWindowBox(wptr,wptr.leftedge,wptr.topedge,270,((pixsep*6)+(mysc.font.ysize*5)+tbar+10))
...
PROC aslhookfunc(hook,fr:PTR TO filerequester,frobj:PTR TO anchorpath)
